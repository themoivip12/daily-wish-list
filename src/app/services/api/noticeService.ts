...
const instance = createServiceFormData(import.meta.env.VITE_APP_API_URL);
...
const publishNovelNotice = async (
  uuid: string,
  reservation: string,
): Promise<any> => {
...
const publishComicNotice = async (
  uuid: string,
  reservation: string,
): Promise<any> => {
  const url = `/v2/charbet-house/comic/notice/${uuid}`;
